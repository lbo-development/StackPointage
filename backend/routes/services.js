...
router.post('/', requireRole('admin_app'), async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from('services')
      .insert({ nom: nom.trim(), code: code.trim().toUpperCase(), description, num_ordre, is_active })
      .select()
      .single();
    if (error) throw error;
...
router.put('/reorder', requireRole('admin_app'), async (req, res) => {
  try {
    ...
    if (!Array.isArray(updates) || !updates.length) {
      return res.status(400).json({ error: 'tableau [{id, num_ordre}] requis' });
    }
    await Promise.all(
      updates.map(({ id, num_ordre }) =>
...
router.put('/:id', requireRole('admin_app', 'admin_service'), async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from('services')
      .update({ nom: nom.trim(), code: code.trim().toUpperCase(), description, num_ordre, is_active })
      .eq('id', req.params.id)
      .select()
      .single();
    if (error) throw error;
...
router.post('/:id/cellules', requireRole('admin_app', 'admin_service'), async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from('cellules')
      .insert({ nom: nom.trim(), code: code.trim().toUpperCase(), couleur, ordre, is_active, service_id: req.params.id })
      .select()
      .single();
    if (error) throw error;
    invalidate(CACHE_PREFIX);
...
router.put('/cellules/reorder', requireRole('admin_app', 'admin_service'), async (req, res) => {
  try {
    const updates = req.body;
    if (!Array.isArray(updates) || !updates.length) {
      return res.status(400).json({ error: 'tableau [{id, ordre}] requis' });
    }
    await Promise.all(
...
router.put('/cellules/:id', requireRole('admin_app', 'admin_service'), async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from('cellules')
      .update({ nom: nom.trim(), code: code.trim().toUpperCase(), couleur, ordre, is_active })
      .eq('id', req.params.id)
      .select()
      .single();
    if (error) throw error;
